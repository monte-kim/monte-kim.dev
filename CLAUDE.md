...
- **KO 본문 지원** (2026-08-03) — `posts.content_ko jsonb` (마이그레이션 `20260803000000_content_ko.sql`, 프로덕션엔 수동 적용됨). 상세 페이지가 로케일로 본문 선택(KO 모드 + content_ko 존재 시 KO, 아니면 EN 폴백; TOC도 동일 문서 기준). **에디터는 이중 언어**: Write 모드 EN/KO 탭이 제목+본문을 언어별로 편집(에디터 인스턴스 1개, `docsRef` 스왑), 자동저장이 양쪽 모두 저장(excerpt_ko 자동 계산 포함), KO가 빈 문서면 null 저장 → 사이트는 EN 폴백. Preview에도 EN/KO 토글. slug/read_minutes는 EN 기준.
...
- **블로그 draft 추가 주입** (2026-08-18, DB 기준 발행 3 + draft 17) — ① Moty 11편째 `moty-refresh-token-race` (EN/KO, 23분, Security+Auth. 근거: 콘솔 토큰 갱신 실작업, 첨삭본 ~/Downloads/moty-dossier/blog/{ko,en}/11-refresh-token-race.md. 코드 대조 검증 완료 2026-08-18 — **발행은 콘솔·서버 해당 수정의 머지+prod 배포 후에만** (7·9·10과 동일 게이트, 12-blog-handoff §1·§2-6), 발행 시 2편 solo-api-contract와 상호 링크) ② Muroom 6편 추가 `muroom-{viewport-search,credential-rotation,file-storage-policy,130-studios-8-users,cold-start-null-result,ai-code-review}` (근거: muroom-backend-bach docs/dossier 브랜치 blog/ 04~09, dfd010f). **Muroom 발행 순서 04→05→06→07→08→09 필수** — 각 편이 앞 편만 크로스 링크하는 구조라 순서 지키면 죽은 링크 없음. 주입 스크립트: 스크래치패드 insert-moty-11.mjs / insert-muroom-4-9.mjs (md→Tiptap 변환기 공용).

- **AI 문체 정리 패스** (2026-08-20, Monte 승인) — ① 사이트 카피: en/ko.json·project-details.ts·OG/메타 태그라인의 산문 em-dash 전량 제거(연도 범위 "2025 — now" 3쌍만 유지), 태그라인 콜론 통일("Production war stories: …"). 이 과정에서 **잔존 사실 오류 3건 교정**: 홈 스탯 183→206(양 로케일), 홈 Muroom 카드 rental·120 endpoints→search·130 studios, About 타임라인 6인·5개월→5인·3주. ② 글 20편 EN/KO 본문: 산문 em-dash EN ≤3/KO 0 수준으로 재구성(11편만 예외 예산), "honest/정직" 틱 편당 ≤1, 대조 구문 재작성 — md 원본 수정 후 DB 전량 PATCH(발행 3편 포함, md↔DB 일치 사전 검증). 딥링크 앵커 6개·코드 블록·수치·EN/KO 구조 패리티 보존 검증 완료. 이후 **딥 패스 v2**(같은 날, Monte 지시로 확장): 제목 20개 재작성(콜론-부제 공식 제거 — 12-blog-handoff §1 표 동기화됨), 본문을 단어·구조·어조 수준으로 재편집(격언체 편당 ≤2, 콜론 연결 과밀 해소, KO 번역투·피동·문두 접속사 정리, 스톡 문구 제거), project-details 상투구 제거. 앵커 대상 EN 헤딩 6개만 단어 동결(구두점 치환은 kebab 불변이라 안전) — 검증 6/6 통과. em-dash 잔존은 코드 블록 내부와 EN 의도적 보존 1~3개뿐. 갱신 스크립트: 스크래치패드 update-posts.mjs(소실 시 재생성 필요 — 변환기는 insert 스크립트와 공용 패턴).

- **전량 발행 완료** (2026-08-25) — 게이트 4편(7·9·10·11)도 Monte의 prod 배포 확인 + 7편 공개 수위 승인으로 같은 날 발행. **DB: 20편 전량 published, draft 0 — 블로그 파이프라인 종료.** 상호 링크 반영: 1편 말미↔9편 서두, 11편 결말→2편(+2편→11 역방향), 4편↔Muroom AWS 글. 남은 발행 관련 작업 없음.
- **13편 일괄 발행 + 연쇄 갱신** (2026-08-25, Monte 지시) — DB 기준 **발행 16 + draft 4**. 발행분: Muroom 04~09 + Moty 1~6·8 (published_at을 시리즈 순서로 1분 간격 부여). 게이트 잔여 4편(Moty 7·9·10·11)은 **prod 배포 확인 후에만**. 연쇄 갱신 완료: fitness 카드 `caseStudy` 활성화, MOTY `caseStudyHref`·`footerCta.seriesHref` 활성화, decisions 딥링크 4개(PII 앵커·pre-agg/dual-DB 앵커·tenant id — OAuth 카드는 해당 글 없어 생략 유지), Moty 4편 서두 ↔ Muroom AWS 글 상호 링크(md+DB 재반영). 프로젝트 OG 카드도 같은 날 추가됨(`/api/og?project=` + 페이지별 meta description). 9편 발행 시 잔여: 1편 말미 "another post" → 9편 링크.

미완료/백로그:
1. ko.json 한국어 카피 검수 (Monte)
2. `/projects` 카드 잔여 작업: **Muroom 문안은 도시에 검증본으로 교체 완료(2026-08-03, 근거: muroom-backend-bach/docs/dossier/10-projects-card.md)**. 남은 것 — ① Muroom GitHub을 org 링크에서 backend 레포로 교체(리소스 해체·키 폐기·공개 전환 후) ② News classifier GitHub 실제 저장소로 (caseStudy 링크 2종은 2026-08-25 활성화 완료)
