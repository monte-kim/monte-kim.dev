...
- **KO 본문 지원** (2026-08-03) — `posts.content_ko jsonb` 컬럼(SQL Editor에서 수동 추가됨, 마이그레이션 파일 없음 주의). 상세 페이지가 로케일로 본문 선택(KO 모드 + content_ko 존재 시 KO, 아니면 EN 폴백; TOC도 동일 문서 기준). **admin 에디터는 EN 본문만 편집** — KO 본문 수정은 스크립트/DB 직접(EN 수정 시 KO 자동 갱신 안 됨). 에디터 EN/KO 탭은 백로그.
