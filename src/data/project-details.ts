...
const MUROOM: ProjectDetail = {
  ...
  rolesSub: {
    ...
    ko: "5인 팀. 저는 제품 전체가 아니라, 회사와 제품의 두 영역을 맡았습니다.",
  },
  roles: [
    {
  ...
  decisions: [
    {
      ...
      body: {
        ...
        ko: "서버비가 사비였기에 관리형 보증을 포기하고 2환경 월 ~$150로 갔고, 그 보증은 손으로 재구축했습니다: 3중 백업, 10초 WAL 쉬핑, 자체 DB 위의 RDS급 로테이션.",
      },
    ...
    },
    {
      ...
      body: {
        ...
        ko: "유료 길찾기는 페이지당 과금, 무료 공공 API는 쿼터로 불가. 기능을 지우고 Java Haversine으로 갔습니다. 정보는 후퇴, 한계비용은 0.",
      },
    ...
    },
    {
      ...
      body: {
        ...
        ko: "리프레시 무효화가 Redis 상태를 요구하는 순간 JWT는 '단계 많은 세션'입니다. 전환을 제안했고, JWT는 단기 핸드셰이크 토큰으로만 남았습니다.",
      },
    ...
    },
    {
      ...
      body: {
        ...
        ko: "앱 생성 64비트 시간 정렬 PK로 INSERT 왕복을 없앴습니다. 이틀 뒤 JS의 2⁵³ 한계가 ID를 조용히 잘랐고, 그날 오후 모든 API ID가 문자열이 됐습니다.",
      },
    ...
    },
    {
      ...
      body: {
        ...
        ko: "실무자 조언에 따라 'DB는 완화, 코드는 강제'. FK 14개를 걷어내고 생성 시점 존재 검증·서비스 소유 연쇄 삭제·partial unique index로 대체했습니다.",
      },
...
const MOTY: ProjectDetail = {
  ...
  product: [
    ...
    },
    {
      ...
      ko: "내부는 이렇습니다: 10Hz 센서 스트림을 쓰기 시점에 사전집계해 분석 쿼리가 원시 하이퍼테이블을 스캔하지 않습니다. 행 팬아웃 ~380× 제거, 쿼리 ~590× 가속(프로덕션 데이터 EXPLAIN ANALYZE 실측). 인증은 grant 6종·scope 27종의 자체 OAuth2형 토큰 서비스입니다. 표준 서버는 PIN·프로필 키오스크 인증을 말하지 못하기 때문입니다. 개인정보는 필드 레벨 암호화(AES-256-GCM + 동등 검색용 블라인드 인덱스)를 9단계 무중단으로 프로덕션에 롤아웃했고, 데이터 계층 자가 호스팅으로 AWS 비용을 58% 줄이며 관리형의 보증은 손으로 재구축했습니다.",
    },
  ],
  ...
  architecture: {
    overview: {
      ...
      ko: "ECS(EC2, Graviton) 위의 Spring Boot 모놀리스 하나가 4개 클라이언트를 서빙합니다. 웹 프론트 2종은 엄격한 BFF라, 브라우저는 API와 직접 통신하지 않고 토큰은 httpOnly 쿠키에만 삽니다. 데이터 계층은 EC2 자가 호스팅: 센서 스트림용 TimescaleDB 하이퍼테이블을 얹은 PostgreSQL 17과, 토큰 상태·rate limit용 Valkey. 배포는 GitHub Actions OIDC(장수명 키 0)에 circuit breaker 롤백, 백업은 교차 리전으로 나갑니다.",
    },
    chain: [
  ...
  rolesSub: {
    en: "Effectively a solo build, with two collaborations I should name.",
    ko: "사실상 단독 구축입니다. 다만 밝혀둘 협업이 두 건 있습니다.",
  },
  roles: [
    {
      title: { en: "Monte (me)", ko: "Monte (본인)" },
  ...
  timeline: [
    ...
    },
    {
      ...
      desc: {
        ...
        ko: "사내 콘솔을 전용 백엔드 API와 같은 스프린트에 1개월로 출시했습니다. 7로케일 콘텐츠 파이프라인, 버전 관리되는 법적 문서.",
      },
    },
    {
      period: "Jun–Jul 2026",
    ...
    },
    {
      ...
      desc: {
        ...
        ko: "필드 레벨 암호화가 9단계 무중단으로 프로덕션에 도달했습니다. 단계마다 추적 가능한 커밋. 임상 스타일 회복 리포트가 뒤따릅니다.",
      },
      mobile: true,
    },
  ],
  ...
  footerCta: {
    title: { en: "The Moty series", ko: "Moty 연재" },
    sub: {
